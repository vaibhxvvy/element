...
const HELP_SECTIONS: &[(&str, &[&str])] = &[
    ...
    (
        ...
        &[
            ...
            "clipboard — cbhist or clip (newest first; add text to filter,",
            "              today / yesterday / YYYY-MM-DD / last7d to filter by date,",
            "              sort:new / sort:old to reorder; Enter copies, right-click pins)",
            "color — #ff0000 or #f00 (copies hex / rgb / hsl)",
            "snippets — snip lists ~/.element/snippets.toml; a name copies it",
            "system — shutdown, restart, sleep, lock, volume 40, mute, screen off,",
            "          timer 10, password, screenshot (captures screen → clipboard + toast)",
    ...
    (
        ...
        &[
            ...
            "Clipboard order (newest/oldest first) is set in settings; sort:new / sort:old override per query.",
